...
def classify(self, dot):
    return 'A' if self.predict(dot) >= 0.5 else 'B'
...
class Perceptron:
    ...
    def train_one(self,input):
        ...
        output = self.predict(dot)
        error = target - output
        delta = error * sigmoid_derivative(output)
        new_weight = []
        for wei,val in zip(self.weights,dot):
            new_weight.append(wei+(self.lr*delta*val))
...
PerA = Perceptron(input_size=2, learning_rate=0.1, max_epoch=10000)
